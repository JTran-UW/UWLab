...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        ...
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                actions = actor(torch.Tensor(obs["policy"]).to(device)).cpu()
                actions += torch.normal(0, args.exploration_noise, size=actions.shape) # actor.action_scale * args.exploration_noise)
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                next_state_actions = (target_actor(data.next_policy_observations) + clipped_noise).clamp(
                    envs.single_action_space.low[0], envs.single_action_space.high[0]
                )
                qf1_next_target = qf1_target(data.next_critic_observations, next_state_actions)
                qf2_next_target = qf2_target(data.next_critic_observations, next_state_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target)
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)

            qf1_a_values = qf1(data.critic_observations, data.actions).view(-1)
            qf2_a_values = qf2(data.critic_observations, data.actions).view(-1)
            ...
            if global_step % (args.policy_frequency * args.num_envs) == 0:
                actor_loss = -qf1(data.critic_observations, actor(data.policy_observations)).mean()
