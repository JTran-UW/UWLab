...
The frame-token path (:meth:`encode_state_frame` + :meth:`forward_from_tokens`) exists for the
online-DAgger student, which keeps a rolling per-env cache of already-encoded state tokens so a
policy step encodes ONE new frame instead of re-encoding the whole window. ``forward`` is exactly
``forward_from_tokens(_encode_states(...))``, so the two paths agree bit-for-bit at equal weights.
"""
...
class HistoryPointNet(nn.Module):
    ...
    def encode_state_frame(self, points: torch.Tensor, proprio: torch.Tensor) -> torch.Tensor:
        """Single-timestep state token: (B,N,point_dim), (B,proprio_dim) -> (B,d_model)."""
        return self._encode_states(points.unsqueeze(1), proprio.unsqueeze(1))[:, 0]
    ...
    # ------------------------------------------------------------------ forward
    def forward_from_tokens(self, state_tok: torch.Tensor, actions: torch.Tensor,
                            valid: torch.Tensor | None = None):
        """Transformer + head over PRE-ENCODED state tokens.

        ``state_tok`` (B,H,d_model) from :meth:`_encode_states` / :meth:`encode_state_frame`,
        ``actions`` (B,H,action_dim) z-scored executed actions (only ``[:, :H-1]`` are consumed),
        ``valid`` (B,H) real-vs-pad mask. Returns per-state-token outputs ``(B,H,action_dim)``,
        or ``(mean, log_std)`` with ``predict_std``. This is the online-DAgger entry point: the
        student caches per-frame tokens so each policy step encodes one new frame only."""
        B, H = state_tok.shape[:2]
        if valid is None:
            valid = torch.ones(B, H, dtype=torch.bool, device=state_tok.device)
        action_tok = self.action_embed(actions)              # (B,H,D)
        seq = self._build_sequence(state_tok, action_tok)    # (B,L,D)
        mask = self._attn_mask(valid, state_tok.device)      # (B*nheads,L,L)
    ...
    def forward(self, points: torch.Tensor, proprio: torch.Tensor, actions: torch.Tensor,
        ...
        return self.forward_from_tokens(state_tok, actions, valid)
