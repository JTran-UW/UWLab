...
def stay_still(
    ...
    cur_hand_joint_pos = asset.data.joint_pos.torch[:, hand_asset_cfg.joint_ids]
    hand_error = torch.norm(hand_command - cur_hand_joint_pos, dim=1)
    des_pos_w, _ = math_utils.combine_frame_transforms(
        asset.data.root_state_w.torch[:, :3], asset.data.root_state_w.torch[:, 3:7], ee_command[:, :3]
    )
    curr_pos_w = asset.data.body_link_pos_w.torch[:, ee_asset_cfg.body_ids, :3].view(-1, 3)
    distance = torch.norm(curr_pos_w - des_pos_w, dim=1)
    goal_reached_mask = (distance < 0.1) & (hand_error < 0.4)

    return torch.where(goal_reached_mask, asset.data.body_vel_w.torch.abs().sum(2).sum(1), 0)
