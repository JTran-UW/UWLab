...
class PartialAssembliesSceneCfg(InteractiveSceneCfg):
    ...
    insertive_object: RigidObjectCfg = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.0, 0.0, OBJECT_SPAWN_HEIGHT * 2), rot=(0.0, 0.0, 0.0, 1.0)),
    )
...
class PartialAssembliesCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # shim). The physics backend is now chosen by assigning a PhysicsCfg
        # subclass to `sim.physics` -- PhysxCfg for PhysX, NewtonCfg for Newton.
        # Field names are unchanged, so this is a straight move.
        #
        # Tuned for contact-rich peg insertion (note the 192 position
        # iterations); these values back the sim-to-real transfer.
