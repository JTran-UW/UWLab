...
class Offset:
    ...
    # Identity quaternion in Isaac Lab 3.0's (x, y, z, w) convention. This was
    # (1, 0, 0, 0) for 2.x's (w, x, y, z); left unchanged it would silently mean
    # a 180-degree rotation about X rather than "no rotation", since the
    # math_utils calls below now interpret the scalar term as the last element.
    ...
    def apply(self, root: RigidObject | Articulation) -> tuple[torch.Tensor, torch.Tensor]:
        # `.torch`: Isaac Lab 3.0 returns a warp-backed ProxyArray here, which the
        # torch.jit.script'd math utils below will not accept.
