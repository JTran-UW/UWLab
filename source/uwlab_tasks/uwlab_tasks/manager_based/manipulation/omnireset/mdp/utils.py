...
def expand_symmetric_offsets(offsets: list[dict]) -> list[dict]:
    """Expand a canonical assembled offset into the 8 poses of a rectangular peg (4 yaws x end-over-end
    flip), matching the yandabao ``assembled_offsets`` convention; the first entry stays canonical."""
    import math

    base = offsets[0]
    bq = torch.tensor(base["quat"], dtype=torch.float64)
    out = []
    for k in range(4):
        a = k * math.pi / 2
        yaw = torch.tensor([math.cos(a / 2), 0.0, 0.0, math.sin(a / 2)], dtype=torch.float64)
        for flip in (False, True):
            g = yaw if not flip else math_utils.quat_mul(yaw.unsqueeze(0), torch.tensor([[0.0, 1.0, 0.0, 0.0]], dtype=torch.float64)).squeeze(0)
            q = math_utils.quat_mul(bq.unsqueeze(0), g.unsqueeze(0)).squeeze(0)
            out.append({"pos": list(base["pos"]), "quat": [float(v) for v in q.tolist()]})
    return out


def get_assembled_offsets(metadata: dict, expand_symmetry: bool = False) -> list[dict]:
    ...
    single ``assembled_offset``, lifted here to a 1-element list. ``expand_symmetry=True`` turns a
    single offset into the 8 rectangular-peg poses (a metadata list is returned as is).
    """
    offsets = metadata.get("assembled_offsets")
    if offsets:
        return list(offsets)
    ...
    if offset is None:
        raise KeyError("metadata has neither 'assembled_offset' nor 'assembled_offsets'")
    return expand_symmetric_offsets([offset]) if expand_symmetry else [offset]
