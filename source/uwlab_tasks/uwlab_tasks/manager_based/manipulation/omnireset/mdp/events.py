...
def randomize_peg_symmetry(
    ...
    """Relabel the peg's pose at reset by a random element of its symmetry group (4 yaws x flip).

    The peg is a rectangular prism whose root frame sits at its geometric centre, so rotating it by
    90 deg about its own axis or flipping it end-over-end yields the SAME physical scene (nothing
    moves, nothing penetrates) with a different pose label. Applied AFTER the dataset reset it
    removes the datasets' canonical-only bias for near-assembled starts, so a symmetric success
    scorer sees all 8 fits equally. ``prob`` = per-env probability of relabeling (identity
    otherwise); ``element`` >= 0 forces one group element (for tests/visualisation); ``allow_flip``
    False restricts to the 4 yaws. Angular velocity is left untouched (zero in the datasets).
    """
    from .symmetric_obs import symmetry_group_local

    asset: RigidObject = env.scene[asset_cfg.name]
    n = len(env_ids)
    group = symmetry_group_local().to(env.device)
    if not allow_flip:
        group = group[0::2]
    if element >= 0:
        k = torch.full((n,), int(element) % group.shape[0], device=env.device)
    else:
        k = torch.randint(0, group.shape[0], (n,), device=env.device)
    q = asset.data.root_quat_w[env_ids]
    q_new = math_utils.quat_mul(q, group[k])
    apply = torch.rand(n, device=env.device) < prob
    q_new = torch.where(apply.unsqueeze(1), q_new, q)
    asset.write_root_pose_to_sim(torch.cat([asset.data.root_pos_w[env_ids], q_new], dim=-1), env_ids=env_ids)
    env.extras.setdefault("log", {})
    env.extras["log"]["peg_symmetry/relabel_frac"] = apply.float().mean().item()
